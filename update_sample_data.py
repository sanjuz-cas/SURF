"""Update database with better sample data matching the UI design."""
import psycopg
import json
...
# First, update raw_feedback with proper categories
updates = [
    (1, 'Bug Fix'),
    (2, 'Bug Fix'),
    (3, 'Feature Request'),
...
for feedback_id, category in updates:
    cur.execute("""
        UPDATE raw_feedback 
        SET category = %s 
        WHERE id = %s
...
# Delete existing prioritized output
cur.execute("DELETE FROM prioritized_output")
...
conn.commit()
print(f"✓ Updated {len(updates)} raw feedback categories")
print(f"✓ Inserted {len(inserts)} new prioritized items")
conn.close()
